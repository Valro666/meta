...
public abstract class AlgorithmeAbstract {
	...
	/**
	 * Nombre d'itération de l'algorithme incrementées dans algo
	 */
	 ...
	 *            solution initiale
	 */
	public AlgorithmeAbstract(ProblemeAbstract probleme, SolutionAbstract solutionInitiale, int maxEvaluations) {
	...
	/**
	 * Exécution complète de l'algorithme - tant qu'il n'y a plus
	 * d'amelioration ou jusqu'a bloquée - ou arriver au nombre max
	 * d'évaluations
	 */
	public void executeJusquaFin() {
		boolean amelioration = true;

		while (amelioration && this.problemeATraiter.getNbEvaluations() < this.maxEvaluations) {
			...
			if (problemeATraiter.getNbEvaluations() % 1000 == 0)
