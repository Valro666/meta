...
public class SolutionTSP extends SolutionAbstract {
	...
	@Override
	/**
	 * retourne les solutions voisines celles ou on a inverse deux villes
	 */
	...
	/**
	 * methode qui retourne un voisin au hasard consiste simplement à retourner
	 * une solution avec une un echange de deux ville au hasard.
	 * 
	 */
	...
	public SolutionAbstract retourneVoisinHasard() {
		...
		return retourneVoisinage().get((int) (Math.random() * retourneVoisinage().size()));

	}

	/**
	 * methode qui retourne une solution perturbée consiste simplement à
	 * retourner une solution avec plusieurs villes echangees.
	 * 
	 * @param n
	...
	public SolutionAbstract retournePerturbation(int n) {
		...
		// TODO a ecrire etudiant
		int size = this.listeVilles.size();

		for (int i = n; i > 0; i--) {
			this.inverse((int) Math.random() * size, (int) Math.random() * size);
		}
